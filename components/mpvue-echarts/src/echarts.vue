	...
	export default {
		...
		methods: {
			init(option) {
				...
				const isValid = version[0] > 1 || (version[0] === 1 && version[1] > 9) || (version[0] === 1 && version[1] === 9 &&
					version[2] >= 91);
				if (!isValid) {
				...
				query
					...
					.boundingClientRect( res => {
						//接受外部传入的属性option初始化后，设置防止再次初始化，接受外部方法传过来的option，进行render
						if (!res) {
							//setTimeout(() => this.init(), 200);  
							return;
						}	
						if (this.loadFlag) {
							this.chart =  this.onInit(canvas, res.width, res.height, this.option);
						}
						this.loadFlag = false;
							if (option && !this.loadFlag ) {
